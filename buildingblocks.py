...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding=1):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            # Add learnable bias only when batch normalization/group normalization is absent.
            bias = not ('g' in order or 'b' in order)
            modules.append(('conv', conv3d(in_channels, out_channels, kernel_size, bias, padding=padding)))
        elif char == 'g':
            is_before_conv = i < order.index('c')
            assert not is_before_conv, 'GroupNorm MUST go after the Conv3d'
            # The number of groups must be less than or equal to the number of channels.
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order='ce', num_groups=8):
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=(2, 2, 2), pool_type='max', basic_module=DoubleConv, conv_layer_order='ce',
                 num_groups=8):
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3,
                 scale_factor=(2, 2, 2), basic_module=DoubleConv, conv_layer_order='ce', num_groups=8):
...
class FinalConv(nn.Sequential):
    ...
    We use (Conv3d+ReLU+GroupNorm3d) by default.
    This can be changed by providing the 'order' argument, e.g. in order
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='ce', num_groups=8):
